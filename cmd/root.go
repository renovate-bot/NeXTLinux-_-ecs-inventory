...
var rootCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		log.Info("Starting nextlinux-ecs-inventory")

		// Check required config values are present
		if appConfig.Region == "" {
			log.Error(
				"AWS region not specified, please set the NEXTLINUX_ECS_INVENTORY_REGION environment variable, use the --region flag, or specify a region in the config file",
...
func init() {
	...
	opt = "region"
	rootCmd.Flags().
		StringP(opt, "r", config.DefaultConfigValues.Region, "if set overrides the AWS_REGION environment variable/region specified in nextlinux-ecs-inventory config")
	if err := viper.BindPFlag(opt, rootCmd.Flags().Lookup(opt)); err != nil {
		fmt.Printf("unable to bind flag '%s': %+v", opt, err)
		os.Exit(1)
	}
